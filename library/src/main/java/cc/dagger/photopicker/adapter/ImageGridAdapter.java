...
import cc.dagger.photopicker.Utils;
import cc.dagger.photopicker.bean.ImageItem;
import cc.dagger.photopicker.view.SuperCheckBox;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (itemViewType == ITEM_TYPE_CAMERA) {
            ...
            if (convertView == null) {
                ...
                convertView.setLayoutParams(new AbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, mImageSize)); //让图片是个正方形
                holder = new ViewHolder(convertView, mImageSize);
...
//                        Toast.makeText(mActivity, mActivity.getString(R.string.select_limit, selectLimit), Toast.LENGTH_SHORT).show();
    ...
    private class ViewHolder {
        ...
        public ViewHolder(View view, int layoutSize) {
            rootView = view;
            mask = view.findViewById(R.id.mask);
            cbCheck = (SuperCheckBox) view.findViewById(R.id.cb_check);
            frame_thumb = (FrameLayout) view.findViewById(R.id.frame_thumb);

            if(ImagePicker.getInstance().getImageLoader() != null) {
                ivThumb = ImagePicker.getInstance().getImageLoader().onCreateGridItemView(view.getContext());
                frame_thumb.addView(ivThumb, new FrameLayout.LayoutParams(
